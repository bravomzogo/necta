...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--force-update", action="store_true", help="Force update even if school already exists")
    ...
    def process_school_page(self, session, link_info, exam, year, all_results, verbose=False, region_name=None, district_name=None, force_update=False):
        ...
        # CHECK IF SCHOOL ALREADY EXISTS FOR THIS EXAM AND YEAR
        ...
        if existing_result and not force_update:
            self.stdout.write(f" ⏭️  Skipping {code} {name} (already exists)")
            return 0  # Skip this school
        ...
        status_indicator = "🔄 Updating" if existing_result else "✅ New"
        self.stdout.write(f" {status_indicator} {code} {name}")
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Scraping finished. Processed {schools_processed} schools, skipped {schools_skipped} schools."))
