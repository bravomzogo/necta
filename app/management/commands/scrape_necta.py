...
# # Use the correct NECTA base URL
...
#             "Kagera", "Mtwara", "Kilimanjaro", "Tabora", "Singida", "Rukwa", "Kigoma",
#             "Shinyanga", "Mara", "Manyara", "Ruvuma", "Lindi", "Pwani", "Geita", "Katavi",
#             "Njombe", "Simiyu", "Songwe", "Iringa", "Mjini Magharibi", "Unguja Kaskazini", 
#             "Unguja Kusini", "Pemba Kaskazini", "Pemba Kusini"
#         ]
...
#         try:
#             resp = requests.get(index_url, timeout=30, verify=False)  # verify=False skips SSL warnings
#             resp.raise_for_status()
#         except Exception as e:
#             raise CommandError(f"Failed to fetch {index_url}: {e}")
...
#                 not href.startswith('index_') and
#                 not href == 'index.htm' and
#                 'indexfiles' not in href):
#                 valid_links.append(link)
        
#         if not valid_links:
#             with open("debug_page.html", "w", encoding="utf-8") as f:
#                 f.write(resp.text)
#             raise CommandError("No school result links found. Saved page content to debug_page.html for inspection.")
...
#             school_url = href if href.startswith(('http://', 'https://')) else f"{BASE_URL.format(year=year, exam=exam)}/{href}"
...
#             try:
#                 sresp = requests.get(school_url, timeout=30, verify=False)
#                 sresp.raise_for_status()
#             except Exception as e:
#                 self.stdout.write(self.style.WARNING(f"⚠️ Failed to fetch {school_url}: {e}"))
#                 continue
...
#             self.stdout.write(f" → {code} {name} (Region: {region}, GPA: {gpa})")
...
#         self.stdout.write("\nRanking schools by GPA (lower is better):")
#         for rank, result in enumerate(all_results, start=1):
#             self.stdout.write(f"{rank}. {result['code']} {result['name']} - {result['region']} - GPA: {result['gpa']}")
